...
with st.form("api_query_form"):
    ...
    with h_col2:
        default_start_date = datetime.date.today() - datetime.timedelta(days=5*365)
        h_start_date = st.date_input("Historical Lookback (Start Date After)*", value=default_start_date, key="h_look")
    ...
    with c_col2:
        ...
        # NOWOŚĆ: Checkbox do filtrowania martwych badań
        c_remove_past_pcd = st.checkbox("Remove trials with Primary Completion Date in the past (or before planned start)", value=True, key="c_pcd")
